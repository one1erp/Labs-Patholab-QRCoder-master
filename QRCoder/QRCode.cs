...
namespace QRCoder
{
    ...
    public class QRCode : AbstractQRCode<Bitmap>, IDisposable
    {
        ...
        public Bitmap GetGraphic(int imageWidth ,int pixelsPerModule, Color darkColor, Color lightColor, Bitmap icon = null, int iconSizePercent = 15, int iconBorderWidth = 6, bool drawQuietZones = true)
        {
            var size = (this.QrCodeData.ModuleMatrix.Count - (drawQuietZones ? 0 : 8)) * pixelsPerModule;
            var offset = drawQuietZones ? 0 : 4 * pixelsPerModule;
          
            var bmp = new Bitmap(imageWidth, size);//, System.Drawing.Imaging.PixelFormat.Format24bppRgb);
            ...
            if (drawIconFlag)
            {
                ...
                //      gfx.DrawImage(icon, iconDestRect, new RectangleF(0, 0, 29, 29), GraphicsUnit.Pixel);
            }
